...
单相配置与训练设置见[训练指南](training.zh-CN.md)。从头训练时，后续阶段使用前一阶段的输出。
...
推理的残差归一化分母下限为 10⁻⁶，有效值阈值为 10⁻⁸；训练分母下限为 10⁻⁸。
